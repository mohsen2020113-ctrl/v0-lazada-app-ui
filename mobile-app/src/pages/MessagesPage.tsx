...
const QUICK_PROMPTS = [
  { en: "Best sellers", ar: "الاكثر مبيعا" },
  { en: "What's on sale?", ar: "ما هي العروض؟" },
  { en: "New arrivals", ar: "وصل حديثا" },
  { en: "Track my order", ar: "تتبع طلبي" },
  { en: "Find a product", ar: "ابحث عن منتج" },
  { en: "Today's deals", ar: "عروض اليوم" },
];

const WELCOME = "Hi! I'm LEE, your personal shopping assistant!" + " " + String.fromCodePoint(0x1F6CD) + "\nmرحبا! انا LEE مساعدك الشخصي للتسوق!\n\nI can help you find any product in our store, check your orders, and answer all your questions.\nيمكنني مساعدتك في ايجاد اي منتج، متابعة طلباتك، والاجابة على استفساراتك.\n\nHow can I help? / كيف يمكنني مساعدتك؟";

export default function MessagesPage(_: Props) {
  const [messages, setMessages] = useState<Message[]>([{
    id: '1', role: 'model', text: WELCOME, timestamp: new Date(),
  }]);
  ...
  const sendMsg = async (text: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        ...
        text: "Connection error, please try again. / خطأ في الاتصال، يرجى المحاولة مجددا.",
        timestamp: new Date()
      }]);
  ...
  const clearChat = () => setMessages([{
    id: crypto.randomUUID(), role: 'model',
    text: "Chat cleared! / تم مسح المحادثة! How can I help? / كيف يمكنني مساعدتك؟",
    timestamp: new Date()
  }]);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-xs text-green-600">Online · Powered by Gemini AI</span>
            </div>
          </div>
        </div>
        <button onClick={clearChat} className="p-2 text-gray-400 active:bg-gray-100 rounded-full">
          <RefreshCw size={18} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        {messages.map(msg => (
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} gap-2`}>
            ...
            {msg.role === 'user' && (
              <div className="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center flex-none mt-1">
